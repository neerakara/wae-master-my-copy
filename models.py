...
# =============================================================================
# network for mapping the inputs to a latent space
# =============================================================================
def encoder(opts, inputs, reuse=False, is_training=False):

    # =========================================================================
    # add noise during training, if required
    # =========================================================================
    if opts['e_noise'] == 'add_noise':
        ...
        def do_nothing(x):
            return x
        # add noiseonly during training
        inputs = tf.cond(is_training,
                         lambda: add_noise(inputs),
                         lambda: do_nothing(inputs))
    ...
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        if opts['e_arch'] == 'mlp':
            ...
            for i in range(num_layers):
                ...
                if opts['batch_norm']:
                    hi = ops.batch_norm(opts, hi, is_training, reuse, scope='h%d_bn' % i)
                hi = tf.nn.relu(hi)            
            if opts['e_noise'] != 'gaussian':
                res = ops.linear(opts, hi, opts['zdim'], 'hfinal_lin')            
            else:
                mean = ops.linear(opts, hi, opts['zdim'], 'mean_lin')
                log_sigmas = ops.linear(opts, hi, opts['zdim'], 'log_sigmas_lin')
                res = (mean, log_sigmas)
        ...
        if opts['e_noise'] == 'implicit':
            ...
            # Now we return res + A(res) * eps, which is supposed to project a noise on the directions depending on the place in latent space
...
# =============================================================================
# network for mapping the latent code to the input space
# =============================================================================
