...
class WAE(object):
    ...
    def __init__(self, opts, train_size=0):
        ...
        # ================================================
        self.reconstructed, self.reconstructed_logits = decoder(opts, noise=self.encoded, is_training=self.is_training)
        
        # ================================================
        # Decode the content of sample_noise
        # ================================================
        self.decoded, self.decoded_logits = decoder(opts, reuse=True, noise=self.sample_noise, is_training=self.is_training)
    ...
    # =========================================================================
    # add placeholders for the real data samples and the noise vector
    # =========================================================================
    def add_inputs_placeholders(self):
        opts = self.opts
        shape = self.data_shape
        data = tf.placeholder(tf.float32, [None] + shape, name='real_points_ph')
        noise = tf.placeholder(tf.float32, [None] + [opts['zdim']], name='noise_ph')
        self.sample_points = data
        self.sample_noise = noise
    
    # =========================================================================
    # add placeholders for hyperparameters (learning rate, etc.)
    # =========================================================================
...
#        self.fixed_noise = self.sample_pz(opts['plot_num_pics'])
        ...
        real_blurr = self.sess.run(self.blurriness, feed_dict={self.sample_points: data.data[:self.num_pics]})
        ...
        for epoch in range(opts["epoch_num"]):
            ...
            if epoch > 0 and epoch % opts['save_every_epoch'] == 0:
                self.saver.save(self.sess,
                                os.path.join(opts['work_dir'],'checkpoints','trained-wae'),
                                global_step=counter)
            ...
            for it in range(batches_num):
                ...
                data_ids = np.random.choice(train_size, opts['batch_size'], replace=False)
                batch_images = data.data[data_ids].astype(np.float)
